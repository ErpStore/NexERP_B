...
{
    public interface IStockIssueRequestService
    {
        ...
        Task<(List<StockIssueRequestVM> minVMs, int TotalCount)> SearchWithDynamicFilterAsync(int pageNumber, int pageSize, Dictionary<string, object>? filters);
        Task<List<string>> GetAllExistingUserNamesinMINAsync();
        Task<IEnumerable<StockIssueRequestVM>> GetAllMINDetailsAsync();
        ...
        Task<List<StockIssueRequestSubVM>> GetAssemblyRelatedItemsAsync(int assyId, decimal ReqQty, int storeId);
        Task<StockIssueRequestSubVM?> GetMINSubItemDetailByMINSubIdAsync(int minSubId);
        Task DeleteAndResequenceAsync(StockIssueRequestSubVM subitem, StockIssueRequestVM minVM, int screenCode);
        Task<List<StockIssueRequestSubVM>> GetMINSubByMINIdAsync(int minId, int storeId);
        Task<StockIssueRequestVM> UpsertMINAsync(StockIssueRequestVM minVM, int screenCode);
        Task<Dictionary<int, decimal>> GetBulkLastUnitPricesAsync(List<int> itemIds);
        Task<StockIssueRequestVM?> GetStockIssueRequestById(int minId);
