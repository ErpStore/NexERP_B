...
{
    public class AssemblyDefService : IAssemblyDefService
    {
        ...
        public AssemblyDefService(IUnitOfWork unitOfWork, ILoggingService logs, 
    ...
    => await _commonService.GetCompanyDetailsAsync();
        ...
        private async Task ProcessAssemblyAsync(List<BomUploadRowViewModel> bomList)
        {
            try
            {
                ...
                int mainSlNo = await _unitOfWork.AssmblyDefs.GetNextSlNoAsync(assyId);
                ...
                foreach (var row in bomList)
                {
                    try
                    {
                        ...
                        {
                            if (await _unitOfWork.AssmblyDefs.ExistsAsync(mainAssyId, itemId))
                                continue;
                        ...
                        else if (mainAssyId == 0)
                        {
                            ...
                            if (await _unitOfWork.AssmblyDefs.ExistsAsync(subAssyId, itemId))
                                continue;

                            int subSlNo = await _unitOfWork.AssmblyDefs.GetNextSlNoAsync(subAssyId);
        ...
        public async Task<(decimal UtilQty, decimal BalQty)> GetUtiliyQtyById(int id)
        {
            ...
            var entity = await _unitOfWork.AssmblyDefs.GetQueryable()
                                  .Where(a => a.Id == id)
        ...
        public async Task<List<AssemblyDefVM>> GetAllAssembliesAsync()
        {
            try
            {
                var entities = await _unitOfWork.AssmblyDefs.GetQueryable()
                    .Include(a => a.AssemblyItem)
                    .OrderByDescending(a => a.AssmblyID)
                    .ToListAsync();

                var result = _mapper.Map<List<AssemblyDefVM>>(entities);
        ...
        public async Task<AssemblyDefVM> GetAssyByAssemblyIdAsync(int assemblyId)
        {
            try
            {
                var entities = await _unitOfWork.AssmblyDefs.GetQueryable()
                ...
                var result = _mapper.Map<AssemblyDefVM>(entities);
            ...
            catch (Exception ex)
            {
                ...
                return new AssemblyDefVM();
            }
        }

        public async Task<List<AssmblyDef>> GetAllAsyItemsByAssyId(int assemblyId)
        {
            try
            {
                return await _unitOfWork.AssmblyDefs
        ...
        public async Task<List<AssemblyDefVM>> GetAssemblyDefByAssemblyIdAsync(int assemblyId)
        {
            try
            {
                var entities = await _unitOfWork.AssmblyDefs.GetQueryable()
        ...
        public async Task DeleteAndResequenceAsync(AssemblyDefVM assemblyDef)
        {
            using var transaction = await _unitOfWork.BeginTransactionAsync();
            ...
            try
            {
                if (assemblyDef.Id > 0)
                {
                    var entity = await _unitOfWork.AssmblyDefs.GetAsync(assemblyDef.Id);
                    if (entity == null)
                        throw new InvalidOperationException("Child item not found.");

                    await _unitOfWork.AssmblyDefs.DeleteAsync(entity);
                    await _unitOfWork.SaveAsync();
                ...
                var remaining = await _unitOfWork.AssmblyDefs
                    .GetQueryable()
        ...
        public async Task UpdateItemCancelAsync(AssemblyDefVM subItem)
        {
            if (subItem == null)
            ...
            try
            {
                var entity = await _unitOfWork.AssmblyDefs
                ...
                await _unitOfWork.AssmblyDefs.UpdateAsync(entity);
                await _unitOfWork.SaveAsync();
            }
            catch (Exception ex)
            {
        ...
        public async Task UpsertBOMAsync(bool isNew, AssemblyDefVM parentAssyVM, List<AssemblyDefVM> bomRows)
        {
            ...
            try
            {
                ...
                if (isNew)
                {
                    ...
                    {
                        var entity = _mapper.Map<AssmblyDef>(row);
                        ...
                        await _unitOfWork.AssmblyDefs.CreateAsync(entity);
                        await _unitOfWork.SaveAsync();
                ...
                {
                var existingBOMs = await _unitOfWork.AssmblyDefs.GetQueryable()
                ...
                if (toDelete.Any())
                {
                    await _unitOfWork.AssmblyDefs.DeleteRangeAsync(toDelete);
        ...
        {
            bool exists = await _unitOfWork.AssmblyDefs.AnyAsync(x => x.AssmblyID == assemblyId);
            return !exists;
        }

        public async Task DeleteAssemblyRelatedDataAsync(int assyId,List<AssmblyDef> allAssemblyRows)
        {
            try
            {
                ...
                await _unitOfWork.AssmblyDefs.DeleteRangeAsync(allAssemblyRows);
                await _unitOfWork.SaveAsync();
        ...
        public async Task ConvertBOMToPRAsync(int mainAssyId, List<AssemblyDefVM> bomRows)
        {
            var now = DateTime.Now;
            var currentUser = await _userService.GetUsernameAsync();
            ...
            try
            {
                ...
                foreach (var row in selectedRows)
                {
                    if (row.Id > 0)
                    {
                        var bomEntity = await _unitOfWork.AssmblyDefs.GetAsync(row.Id);
                        if (bomEntity != null)
                        {
        ...
        public async Task<SCNGenVM> AddStockFromBOM(int assemblyId,int? subAssyId, int? subAssyId2, int? subAssyId3, List<AssemblyDefVM> bomRows)
        {
            using var transaction = await _unitOfWork.BeginTransactionAsync();
            ...
            try
            {
                ...
                {
                    var existingItem = await _unitOfWork.AssmblyDefs.GetAsync(row.Id);
                    if (existingItem != null)
                    {
                        existingItem.BalQty -= row.ReqQty.GetValueOrDefault();
                        existingItem.StockConvertedQty += row.ReqQty.GetValueOrDefault();
                        await _unitOfWork.AssmblyDefs.UpdateAsync(existingItem);
                        await _unitOfWork.SaveAsync();
                    }
                }
        ...
        private async Task<SCNGen> CreateSCNGenAsync(int assemblyId,int? subAssyId, int? subAssyId2, int? subAssyId3, List<AssemblyDefVM> bomRows)
        {
        ...
        public async Task<(List<AssemblyDefVM> assemblyDefVMs, int TotalCount)>
SearchWithDynamicFilterAsync(int pageNumber, int pageSize, Dictionary<string, object>? filters)
        {
            try
            {
                IQueryable<AssmblyDef> baseQuery = _unitOfWork.AssmblyDefs.GetQueryable();
                ...
                var list = await _unitOfWork.AssmblyDefs
                    .GetQueryable()
                ...
                var vmList = _mapper.Map<List<AssemblyDefVM>>(list);
        ...
        {
            public static IQueryable<AssmblyDef> ApplyFilter(
                IQueryable<AssmblyDef> query,
                string field,
                object value)
            {
                if (value == null)
        ...
        public async Task<(bool CanDelete, string Message)> CanDeleteAssydefAsync(int id)
        {
            try
            {
                var assmblyDef = await _unitOfWork.AssmblyDefs
                    .GetQueryable()
        ...
        public async Task<bool> DeleteBOMAsync(int bomId)
        {
            try
            {
                var assmblyDefs = await _unitOfWork.AssmblyDefs
                    .GetQueryable()
                    .Include(x => x.AssemblyItem)
                ...
                var assemblyCode = assmblyDefs.FirstOrDefault()?.AssemblyItem?.ItemCode ?? "";
                var totalRows = assmblyDefs.Count;

                await _unitOfWork.AssmblyDefs.DeleteRangeAsync(assmblyDefs);
                await _unitOfWork.SaveAsync();
        ...
        //--SNS---
        public async Task<AssmblyDef> GetByAssemblyAndItemAsync(int assemblyId, int itemId)
        {
            return await _unitOfWork.AssmblyDefs
