...
{
    public class AssemblyRepository : Repository<AssmblyDef>, IAssemblyRepository
    {
        ...
        public override async Task<IEnumerable<AssmblyDef>> GetAllAsync(bool asNoTracking = false)
        {
            return await _db.AssmblyDef
                .AsNoTracking()
        ...
        public async Task<AssmblyDef?> GetByAssmblyIdAsync(int assmblyId)
        {
            return await _db.AssmblyDef
                .FirstOrDefaultAsync(x => x.AssmblyID == assmblyId);
        }

        public override async Task<AssmblyDef> UpdateAsync(AssmblyDef obj)
        {
            try
            {
                var tracked = await _db.AssmblyDef.FirstOrDefaultAsync(c => c.Id == obj.Id);
                if (tracked == null)
                    return new AssmblyDef();
