...
{
    public class ProductionSCNAssyRepository :Repository<ProductionSCNAssy>, IProductionSCNAssyRepository
    {
        private readonly ApplicationDbContext _db;
        private readonly ILoggingService _logs;
        public ProductionSCNAssyRepository(ApplicationDbContext db, ILoggingService logs) : base(db, logs)
        {
            _db = db;
            _logs = logs;
        }
        ...
        public async Task<string> GetLastSCNNoAsync(string suffix)
        {
            ...
            var lastNumberStr = await _db.ProductionSCNAssy
                .FromSqlRaw(@"
            SELECT TOP 1 * 
            FROM ProductionSCNAssy WITH (UPDLOCK, ROWLOCK)
            WHERE Suffix = {0}
