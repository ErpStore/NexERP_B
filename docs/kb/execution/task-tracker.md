...
| M2-A11    | M2        | Re-specify `M2-A05` for Angular                                   | Documentation | **Completed**¹¹²˒¹¹⁵˒¹¹⁶ _(implemented and merged to `master` `0d313d5` 2026-08-27, carried in on `M2-A05`'s merge since it was stacked underneath — corrected `M2-A05.md`'s stale React-era content, its false "`M2-C02` implements the tenant picker" claim, and every `/api/auth/` route to `/api/v1/auth/`. Owner confirmed `Completed` 2026-08-27. See footnotes ¹¹² and ¹¹⁶)_             | P0       | —                                                              | 0.5 d    | G2   |
...
**Downstream effect — corrected, see footnote ¹¹⁰.** The claim first written here ("nothing
currently names `M2-C03` in `depends_on`") was wrong: it was checked only against
`task-tracker.md`'s own Dependencies column and prose, not against every task file's own
frontmatter. `tasks/M2-C08-01.md` and its parent `tasks/M2-C08.md` both name `M2-C03` as a
Hard `depends_on` entry (added by the same `Q-102` pass that corrected `M2-D01`'s). Rule 1 of
the five-part "can actually be done" test requires `Completed` **and** merged, not merely
merged — `M2-C03` stays `Needs Review` until the owner reviews the Close-out
([`tasks/M2-C03.md`](tasks/M2-C03.md)) and explicitly sets `Completed`, so `M2-C08-01` does
**not** actually clear rule 1 yet, despite its row below reading `Ready`. See footnote ¹¹⁰.

¹¹⁰ **`M2-C08-01`: `Ready` → `Blocked`, 2026-08-27, correction found during a select-only
pass looking for the next dispatchable task after `M2-C03`'s merge.** Footnote ¹⁰⁴ marked
this row `Ready` on 2026-08-27 by checking only its one named Dependencies-column entry,
`M2-C07` (`Completed`). `tasks/M2-C08-01.md:12`'s own frontmatter `depends_on` — corrected by
the same `Q-102` pass that fixed `M2-D01`'s (2026-08-26) — actually names five Hard
prerequisites: `M2-C07`, `M2-C04-02`, `M2-C04-03`, `M2-C03`, `M2-C02`. Checked all five just
now: `M2-C07`, `M2-C04-02`, `M2-C04-03`, `M2-C02` are all `Completed` and merged; **`M2-C03`
is `Needs Review`, not `Completed`** (footnotes ¹⁰⁸–¹⁰⁹). Rule 1 of the five-part "can
actually be done" test requires `Completed` **and** merged, so `M2-C08-01` does not clear it.
Row and Dependencies column above corrected to `Blocked` and to list all five Hard deps, not
just one. No branch was cut and no code was written for this task.

¹¹¹ **`M2-D01`: dispatch attempted, stopped `Blocked` on arrival, 2026-08-27 — same
select-only pass, master tip `f235aeb`.** `M2-D01`'s frontmatter `depends_on` (`M2-C05-03`,
`M2-A02`, `M2-B10`, `M2-C02`, `M2-A07`, `M2-A06`, `M2-B01`) is genuinely all `Completed` and
merged — row 180's `Ready` tag is correct on rule 1. But `tasks/M2-D01.md`'s own
_Prerequisites_ section lists a second, "transitively required" table the task explicitly
says to check before starting, "because each is a silent blocker," with its own instruction:
"If any of these is incomplete, stop and report Blocked. Do not stub the missing piece."

Checked every row in that table. All but one are `Completed`/merged (`M2-C01`, `M2-C02`,
`M2-C03` — merged, sufficient for this table's purpose since it is not the formal rule-1
gate — `M2-C04-01/-02/-03`, `M2-C05-01`, `M2-A04`, `M2-A06`, `M2-A07`, `M2-B01`, `M2-B02`,
`M2-B03`, `M2-B07`). **`M2-A05` — "Cross-origin SPA tenant resolution + real CORS" — is
`Blocked`, `status: Not Started`, no work done.** Confirmed this is a real, live blocker, not
a stale tag: `V.SMART/V.SMART.Api/Program.cs:166-169` hardcodes
`policy.WithOrigins("http://localhost:4200")` for the `"AngularDev"` CORS policy — the
`frontend-new` e2e dev server runs at `127.0.0.1:4300`
(`frontend/nexgen-web/playwright.config.ts:3`), a different origin the current policy does
not allow. `M2-D01`'s own Objective requires "login → tenant resolution → ... runs
end-to-end for a real screen," and its Testing Requirements name an e2e case explicitly
"against a real API and a non-production tenant" — both genuinely blocked by this CORS gap,
not merely inconvenienced by it. Fixing it is out of `M2-D01`'s scope
(`V.SMART/V.SMART.Api/Program.cs` is under _Files That Must Not Change_ — "A needed change is
a finding to report, not an edit"), so per the task's own instruction this is reported, not
stubbed. No branch was cut and no code was written for this task. `M2-D01`'s tracker row
stays `Ready` on its formal rule-1 reading — the blocker is task-file-specific, not a
tracker-status error like `M2-C08-01`'s — but it cannot actually be dispatched until
`M2-A05` lands.

**Net result of this pass: no task currently clears its own full prerequisite check.**
`M2-C08-01` fails rule 1 directly (`M2-C03` not `Completed`). `M2-D01` clears rule 1 but
fails its own file's explicit secondary check (`M2-A05` not done). The real next blocker on
...
¹¹⁶ **`M2-A05` and `M2-A11` both set `Completed`, 2026-08-27, on explicit owner
instruction** ("okay mark it as completed" — clarified via a direct question in-conversation
to cover both tasks, since both were merged together in `0d313d5` and both sat at `Needs
Review`). Per [`workflow.md`](workflow.md#who-may-set-completed) (KB-088), this session
never infers `Completed` from "merged" alone — the merge (footnote ¹¹⁵) and its post-merge
re-verification were reported first, and this status change is recorded here only because
...
¹¹⁸ **`M2-C03` set `Completed`, 2026-08-27, on explicit owner instruction** ("yes mark it as
completed", after being walked through the Close-out's full acceptance-criteria and
disclosed-gaps detail in-conversation). Per
[`workflow.md`](workflow.md#who-may-set-completed) (KB-088), this reflects the owner's
sign-off, not that every criterion is met without qualification — the four `[~]` criteria
(no automated `axe` scan; `/dashboard`-only route verification; the `sm`-band drawer
untested at a real narrow viewport; intermittent full-suite noise traced to workstation
load, not a leak) are accepted as known, disclosed limitations, not retracted.

**Downstream effect, now real — both prior blockers this footnote chain named are
cleared.** Rule 1 of the five-part test now passes for both dependents:
- **`M2-C08-01`** — all five Hard `depends_on` rows (`M2-C07`, `M2-C04-02`, `M2-C04-03`,
  `M2-C03`, `M2-C02`) are `Completed` and merged. `Blocked` → `Ready` above.
- **`M2-D01`** — its own stricter "transitively required" table (footnote ¹¹⁷) now clears in
  full: `M2-A05` and `M2-C03`, its last two open rows, are both `Completed` and merged.
  `Blocked` → `Ready` above.

**Selection, per [`dependency-graph.md`](dependency-graph.md) § *Selecting the next task*,
ranked just now:** both candidates are `P0` (rank 1 ties). Rank 2, most downstream
unblocking: `M2-D01` has two direct dependents in the tracker's own Dependencies column
(`M2-D02`, `M2-D02-01`); nothing currently names `M2-C08-01` there (its siblings
`M2-C08-02`/`-03` depend on the parent container `M2-C08`, not on `M2-C08-01` specifically).
`M2-D01` wins outright at rank 2. It is also independently on the critical path
(`dependency-graph.md:212`), while `M2-C08-01` leads to `M2-C08-02`/`-03` and `M3-5`, a
separate branch — rank 3 agrees. **`M2-D01` selected.** Not dispatched by this footnote
alone — task selection is written to `current-task.md`; branch creation and implementation
remain separate steps, per convention. A branch from `M2-D01`'s prior stopped attempt
already exists (`migration/M2-D01-currency-end-to-end`, footnote ⁷⁸) and is the natural
one to resume, not a conflicting sibling.
...
**Merged to `master` 2026-08-28** (`9dbb9bb`, `--no-ff`) on explicit owner instruction ("merge
M2-C08-01 to master"). Clean merge, no conflicts — the branch had already taken `master` in at
`12df786`, which is where the conflicts were resolved. Re-verified on merged `master`:
`format:check`, `lint` and `typecheck` clean, **734/734 unit tests over 107 files, exit 0**,
`build` succeeds; the bundle figure is identical before and after the merge (644.91 kB, 44.91
kB over budget — `M2-D01`'s overage, not this task's).

**Still `Needs Review`, not `Completed`.** The owner instructed the merge, not the sign-off;
only the owner may set `Completed` ([`workflow.md`](workflow.md#who-may-set-completed)).
**`M2-C08-02`** (server-authoritative totals wiring) and **`M2-C08-03`** (workflow command
