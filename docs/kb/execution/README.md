...
> [KB-106](../security/default-admin-removal-runbook.md) per tenant (new open question **Q-25**),
...
| 3 | No-rights user refused by the **API**, proven by a CI permission-matrix harness | **Half met — harness merged 2026-08-25 (`M2-A03`, `d94d8ce`): 470 → 508 Api tests, no enforcement holes found. What remains is **not code**: the CI job must be marked a *required* status check in GitHub branch protection, which no execution session can set. Original assessment:**  Enforcement exists and is tested — `M2-A01-03`, `M2-A08` (`ScreenRightAuthorizationFilterTests`, `RowScope*Tests`). The **harness does not**: `grep` for `permission-matrix` finds only a comment in `MeEndpointTests.cs:396` noting `M2-A03` has not landed. `M2-A03` is `Blocked` behind `M2-A02`. |
| 4 | TypeScript client generated from OpenAPI in CI | ✅ **MET 2026-08-25 by [`M2-B10`](tasks/M2-B10.md)**, merged. `api/openapi.json` and 62 files under `frontend/nexgen-web/src/app/core/api/generated/` are committed, and a CI job regenerates and diff-checks both so a hand-edit cannot land. It also closed KB-114 §13's annotation gaps first — 45 `[ProducesResponseType]` added — so the client does not inherit a contract missing the 404 on `GET /currencies/{id}` and the 409 on `DELETE`. |
| 5 | Parity test `M2-D03` passes | **Not met.** `M2-D03` is `Blocked`; its spec was only made implementable on 2026-08-23 by `M2-C12-05`. |
| 6 | Controller template + error contract documented and adopted | **Half met — template merged 2026-08-25 as [KB-114](../api/controller-conventions.md) (`M2-B03`), and `M2-B10` then brought both controllers into conformance on `[ProducesResponseType]`. What remains for "adopted" is a judgement on whether two controllers constitute the DoD's "two independent users". Original assessment:** The error contract landed and is adopted by every controller (`M2-A06`, `problem+json`). The **controller template** is owned by [`M2-B03`](tasks/M2-B03.md) *(“Codify the controller template and API conventions”)*, `Blocked` on `M2-A02`. Its own Definition of Done requires "two independent users" before it counts. |
