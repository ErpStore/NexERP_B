...
| Field | Value |
|---|---|
| **Status** | `RUNNING` — autonomous run started 2026-08-20 by owner instruction (`/migration-runner`). **Two branches are validated `PASS` and awaiting owner merge, and neither is on `master`:** `M2-C00` (`migration/M2-C00-kb050-angular-rewrite`, `b3c0e6e`) and `M2-A07` (`migration/M2-A07-me-endpoint`, `61da4bd`). `master` had both recorded as `Ready` — corrected in this commit, because a fresh branch cut from `master` would otherwise have re-implemented finished work. Neither is selectable; neither releases its dependents until merged ([selection rule](dependency-graph.md#ready-task-selection-rule) step 1). 
...
| **Current task** | None yet — awaiting the Select phase of this run. `M2-C00` is **not** a candidate (done, unmerged); `M2-C01` is `Blocked` behind it. 
...
| **Attempt** | n/a — no task in flight. Last closed: `M2-A01-03`, **2 of 3** used, `PASS`, now `Completed` and merged. *(The count read "2 of 4" — the retry-budget denominator wrong for the sixth time. It is **three**: KB-091 §6.4 "Attempt 3 fails → BLOCKED … Do not attempt a fourth", and `migration-runner.js` `maxRetries: 2`.)* |
| **Escalations** | 0 |
...
| **Next ready task** | **Not pre-selected — re-apply the [selection rule](dependency-graph.md#ready-task-selection-rule).** Excluded from the candidate set: `M2-C00` and `M2-A07` (both done, validated `PASS`, `Needs Review`, unmerged — see **Status**), `M2-C01` (`Blocked` behind `M2-C00`), `M2-C04-02` (**void** — [ADR-007](../decisions/ADR-007-angular-stack.md) replaced React with Angular on 2026-08-20 and every `M2-C*`/`M2-D*` task file carries a ⛔ STOP banner pending re-specification), and `M2-A02` (`Ready` but gated on the unanswered **Q-28**). Genuine candidates: `M2-A08` (P0 — read **R-44**/**Q-37** first), `M2-B12-01`, `M2-B09`, `M2-B04`, `M0-01-03`. 
