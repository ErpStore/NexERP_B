...
| Field | Value |
|---|---|
| **Status** | `RUNNING` — resumed 2026-08-21 by owner instruction (`/migration-runner`); closed `M2-B04`; halted at Select with an empty candidate set; the owner then **authorised `M0-01-03`'s executable half**, which ran and closed `Needs Review`. halted twice at Select with an empty pool; **the owner then merged `M2-B01` (`ae9d2c8`), which released three tasks**, and `M2-B09` was executed to completion and closed `Needs Review` (`d1175db`). Owner: **Vivek**. **The `M0-01-03` stop turned out to be softer than §8 item 5 implied** — its environment half was simply stale, and running it produced **R-65**, a silent-lockout defect that blocks `M2-A02`. **Selectable now:** `M2-B06` (P1, 1 wk) and `M2-B11` (P2, 3 d). **Six branches carry a claimed `PASS` and await owner merge, none on `master`** — `M2-B09` (`…-reference-endpoints`, `d1175db`), `M2-B04` (`…-decouple-pages-references`, `5ca1c10`), `M2-A08` (`…-row-scope-and-account-gates`, `bca92fd`), `M2-A07` (`…-me-endpoint`, `e3bc96c`), `M2-C00` (`…-kb050-angular-rewrite`, `b3c0e6e`) and `M0-10` (`…-candelete-guard-audit`, `fc8e0c0`) — **plus `M0-01-03` (`34b5e32`) awaiting a named operator, and `M2-B12-01` `Blocked` on Vivek with its escalation budget exhausted.** None releases its dependents until merged ([selection rule](dependency-graph.md#ready-task-selection-rule) step 1). Every branch state in this row was read from the branch itself — see **Process note — inherited status** below for why that mattered. |
