...
| Field | Value |
|---|---|
| **Status** | `STOPPED` — no run is live. **`M2-A01-03` is `Completed` and merged** (`edcf126`, owner-instructed 2026-08-20) after validating `PASS`; `master` pushed at `edcf126`. That merge released **`M2-A02`** (gated on **Q-28**), **`M2-A07`** and **`M2-A08`**. 
...
| **Next ready task** | **Not pre-selected — re-apply the [selection rule](dependency-graph.md#ready-task-selection-rule).** This row previously named `M2-C04-02`; that selection is **void**. [ADR-007](../decisions/ADR-007-angular-stack.md) replaced React with Angular on 2026-08-20, and `M2-C04-02` is now `Blocked` behind **`M2-C00`** and carries a ⛔ STOP banner. **All 26 `M2-C*`/`M2-D*` task files carry that banner: selecting one means stop and report, never infer the missing specification.** Current `Ready` set: **`M2-C00`** (P0, gates all 20 frontend tasks — rewrite KB-050 for Angular), `M2-A07`, `M2-A08`, `M2-B12-01`, `M2-B09`, `M2-B04`, `M0-01-03`; and `M2-A02`, which is `Ready` but **gated on Q-28** — an API-only administrator holds zero `UserRight` rows. 
