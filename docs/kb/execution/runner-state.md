...
| **Attempt** | `M0-01-03`: **0 of 3** used — not started. `M2-B04` (closed): **2 of 3** used, 0 escalations, verdict `PASS`, status `Needs Review`. `M2-B12-01`: **2 of 3** used, **1 of 1 escalations spent**, verdict `FAIL` at `fa4a2ad`, status **`Blocked`** — *not* `PASS`, and the escalated fix at `8a54f96` has never been re-validated. `M2-A08`: **1 of 3** used, 0 escalations, `PASS`, `Needs Review` on `…-row-scope-and-account-gates`. |
| **Escalations** | 0 |
