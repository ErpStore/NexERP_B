...
| **Blocked on** | **`M0-12-01`, on the repository owner** — a decision, not a diagnosis: either (A) explicitly authorise pushing `migration/M0-12-01-test-project` this conversation so CI can be observed going red on a deliberately-broken assertion and reverted, or (B) waive acceptance criterion 6 for this task, consistent with `M0-07` having been signed off `Completed` with the identical gap open (`d79e1a4`). Also still human-blocked: `M0-04` `Blocked`⁴ (unidentified owner, production SQL / GST gateway access — also blocks `M0-05`); `M0-01-03` `Needs Review` (repo-side work merged; only a human-executed rebuild drill remains, a hard G0 exit criterion). |
