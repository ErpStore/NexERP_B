...
| **Run the API tests** (M2-A06) | `dotnet test tests/V.SMART.Api.Tests/V.SMART.Api.Tests.csproj` | **21 tests discovered, 21 passed, 0 failed, 0 skipped**; test execution ~0.1s, whole command ~20s warm (measured 2026-08-20, M2-A06, locally, on branch `migration/M2-A06-problem-details`). Project created by M2-A06, not by M2-A02 as earlier planned — M2-A02 had not landed. No database and no host: the error contract is exercised over `DefaultHttpContext` and controllers with stand-in services. Like the Shared suite, do **not** pass `--no-build`/`--no-restore`. ~~Not yet wired into `NexGen-ERP---2025-master.sln` or `.github/workflows/ci.yml`~~ — **no longer true**: verified 2026-08-24 (M2-A02) at `NexGen-ERP---2025-master.sln:18` and `.github/workflows/ci.yml:213-217` (step *Test - V.SMART.Api.Tests*). It gates merges. Re-measured 2026-08-20 on `migration/M2-A01-02-require-screen-right`: **104 tests discovered, 104 passed, 0 failed, 0 skipped**, execution 123 ms — the growth is M2-A01-02's filter and startup-validator suites. Re-measured 2026-08-20 on `migration/M2-A01-03-rights-cache` after the attempt-1 repair: **117 tests discovered, 117 passed, 0 failed, 0 skipped**, execution ~1 s — the growth is M2-A01-03's 13 cache tests, and the ~1 s is the one deliberate wall-clock test that watches a 1-second TTL expire. Re-measured 2026-08-20 on `migration/M2-A07-me-endpoint`: **148 tests discovered, 148 passed, 0 failed, 0 skipped**, execution 1 s — the growth is M2-A07's 31 `/api/v1/me` tests. Re-measured 2026-08-24 on `migration/M2-A02-currency-authorization`: **357 tests discovered, 357 passed, 0 failed, 0 skipped**, execution 6 s — 45 of them are M2-A02's `CurrencyAuthorizationTests`; the rest of the growth came from M2-A08/M2-B06/M2-B11 landing in between. Re-measured 2026-08-24 on `migration/M2-A03-permission-matrix-harness`: **470 tests discovered, 470 passed, 0 failed, 0 skipped**, execution 6 s — the growth is M2-A03's 106 permission-matrix harness tests, of which 60 are the generated `(gated endpoint x fixture)` matrix. The harness needs no separate invocation: it lives in this project and inherits this command and the CI step. The whole harness alone can be run with `--filter "FullyQualifiedName~PermissionMatrix"` (verified 2026-08-24: 106 tests, ~4-5 s). A single class can be run with `--filter "FullyQualifiedName~CurrencyAuthorizationTests"` (add `--no-build` only when the build is already warm). **Still controller-level only**: the project has no `Microsoft.AspNetCore.Mvc.Testing` reference and no host, so nothing here asserts an HTTP status over the wire (R-43) |
| **Generate the OpenAPI contract** (M2-B10) | `bash tools/generate-openapi.sh` (or the full `bash tools/generate-api-client.sh`, which also regenerates the Angular client; add `--check` for the CI drift check) | **Verified 2026-08-24 (M2-B10, locally, Git Bash on Windows).** Writes `api/openapi.json` — 18 operations, 6 controllers, 17 schemas — via `dotnet swagger tofile` (Swashbuckle.AspNetCore.Cli 7.3.1, pinned in `.config/dotnet-tools.json`; `dotnet tool restore` first, which the script does). **Byte-identical on repeated runs** (`md5sum` equal across two consecutive runs and across a rebuild), which is what makes the CI drift check trustworthy. The document validates against the OpenAPI schema (`@apidevtools/swagger-parser` `validate()`: *VALID OpenAPI: V.SMART API v1*). **The four environment variables the script sets are placeholders, not secrets** — `dotnet swagger tofile` builds the host, so `StartupConfigurationValidator` demands `ConnectionStrings__MasterDb`, `Jwt__Secret` (≥32 bytes), `Jwt__Issuer` and `Jwt__Audience`; nothing opens a database connection, so fake values work (Confirmed by running it). Without them the failure is the **misleading** `"A type named 'StartupDevelopment' or 'Startup' could not be found"` — Swashbuckle's fallback after the host factory threw. Full detail: [KB-112](../api/generated-client.md) |
| **Regenerate contract + client, and check for drift** (M2-B10) | `bash tools/generate-api-client.sh [--check]` | **Verified 2026-08-24 (M2-B10, locally), all three outcomes observed:** clean tree → exit 0, `no drift: the committed contract and client match the API`; one operation id renamed in a controller → exit **1** with the fix command and the diff; a hand-edited generated file (staged, i.e. what CI sees) → exit **1**. Requires `npm ci` in `frontend/nexgen-web` first (`ng-openapi-gen` is a devDependency, invoked with `npx --no-install`). It is the job `api-contract` in `ci.yml` — **the job itself has never run on a GitHub runner**; no execution session may push |
| **Run the tests** (M0-12-01) | `dotnet test tests/V.SMART.Shared.Tests/V.SMART.Shared.Tests.csproj` | **11 tests discovered, 11 passed, 0 failed, 0 skipped**; test execution 9s, whole command ~15s warm (measured 2026-08-19, M0-12-01, locally, SDK 10.0.400). Do **not** add `--no-build` or `--no-restore`: this is the only project that restores it. It is currently the repository's *only* test project — a `dotnet test` that reports fewer than 11 tests has targeted the wrong path. **Count amended 2026-08-19 (M0-06): the suite is now 85 tests, 85 passed, 0 failed, ~10s** — M0-12-02, M0-13, M0-09 and M0-06 each added to it; treat 11 as the historical floor, not the expected count |
| **Add an EF migration** (verified 2026-08-19, M0-06) | `ConnectionStrings__DesignTimeTenantDb=<any value> dotnet ef migrations add <Name> --project V.SMART/V.SMART.Shared/V.SMART.Shared.csproj --framework net9.0 --context ApplicationDbContext` | **Works.** `dotnet-ef` 10.0.11 on SDK 10.0.400 drives the EF Core 9.0.5 model; "Build succeeded… Done." Both switches are **required**: without `--framework net9.0` the multi-targeted project is ambiguous, and without `--context` the tool exits with *"More than one DbContext was found."* The environment variable must be **set** (`Data/MigrationData/DesignTimeConnectionString.cs:23-62` throws otherwise) but **no connection is opened** — `migrations add` needs no reachable database and no real credential. Review the generated file: a `HasData` removal scaffolds `DeleteData`, and the paired `Down()` re-inserts the removed literal values |
