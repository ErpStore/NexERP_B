...
> [KB-104](../security/default-admin-removal-runbook.md).
...
> **Open — Q-88.** BR-DOC-001 omits the `+1` that every other branch applies, then writes the
...
> source** — see [`open-questions.md`](../open-questions.md) Q-88. It is recorded as observed
...
- **Gap-avoiding decrement on delete.** When the deleted document held the current high-water
  ...
  `LastNumber > 1`; the other six can write `LastNumber = 0` (**Q-86**, intent Unknown).
