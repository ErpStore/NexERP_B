...
| ✅ **RESOLVED 2026-08-24 (M2-A09).** `ScreenCatalogue.cs` now lists **150** names; the two phantoms are deleted, not merely flagged. `[RequireScreen("Bill Paid List")]` now fails startup validation loudly instead of passing it and denying every request forever. | This closed the silent lockout warned about at `:130` of this document. Tracked as **R-65**, resolved in [KB-060](../risks/technical-debt-register.md). | **Confirmed** |
