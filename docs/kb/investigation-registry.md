...
| INV-032 | Decimal representation across the HTTP wire — format, precision source, rounding mode | M2-C10 | **Claimed and `Complete` 2026-08-23 — moved to the Completed table above.** Wire format **Inferred** (JSON number, reasoned from the Confirmed absence of any serialiser override; **not observed live** — raised as **Q-77**), precision sources Confirmed, rounding mode Confirmed (`MidpointRounding.AwayFromZero`), so no KB-004 question for the rounding mode. |
...
| **INV-048 +** | **next free** *(INV-042 was double-claimed by `M2-A07` and `M2-B04`; resolved at merge 2026-08-21 — M2-A07 keeps INV-042, M2-B04 became INV-047. INV-045 was claimed by M2-B06 and INV-046 by M2-B11. Check `git branch --no-merged master` before claiming)* | — | — |
