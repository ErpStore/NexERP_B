...
| INV-032 | Decimal representation across the HTTP wire — format, precision source, rounding mode | M2-C10 | **Claimed and `Complete` 2026-08-23 — moved to the Completed table above.** Wire format Confirmed-by-absence-of-override (JSON number; not observed live), precision sources Confirmed, rounding mode Confirmed (`MidpointRounding.AwayFromZero`) — no Unknown, so no KB-004 question. |
