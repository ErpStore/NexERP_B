...
| INV-032 | Decimal representation across the HTTP wire — format, precision source, rounding mode | M2-C10 | **Still reserved, but no longer un-answerable — 2026-08-25.** The wire-format half is **already measured** and committed: `INV-051` (Complete, 2026-08-24, `M2-B10`) established `decimal` → `type: number, format: double` → TypeScript `number`, and explicitly routed the consequence here rather than resolving it. No live endpoint is needed to record it; the evidence is `api/openapi.json` and the generated client at `HEAD`. See **Q-85** for the decision that half raises. What remains genuinely open is the *precision source* and *rounding mode*, which are server-side and independent of the wire. |
