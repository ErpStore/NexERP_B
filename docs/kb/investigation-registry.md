...
| INV-027 | Stored-procedure DDL capture (all 94) | **Partial** (repository half complete 2026-08-13, M0-01-01; **capture tooling added 2026-08-13, M0-01-02 half A — the DDL itself is still uncaptured**) | The reference-vs-scripted reconciliation is done with no database access: 94 referenced, 13 declared, 11 `scripted`, 1 `case_mismatch` (`Sp_Print_MFGDC` vs. `Sp_Print_MfgDC`), 1 `unreferenced` (`Sp_Print_PurchaseOrder`, dead DDL), **82 `missing`** — the corrected figure, re-deriving and confirming R-04's existing "82" independently. Worklist: `db/stored-procedures/manifest.csv`. Reproducible via `db/tools/sp-inventory.sh` (scoped grep: `grep -rhoE "Sp_[A-Za-z0-9_]+" --include=*.cs --include=*.razor --exclude-dir=obj --exclude-dir=bin V.SMART \| sort -u`). Negative results recorded: no procedure name is referenced only from commented-out code; no evidence of a run-time-composed (string-concatenated/interpolated) procedure name anywhere a `.cs`/`.razor` grep can see. **Remaining gap, live-database half, owned by M0-01-02:** the DDL itself has not been captured — this method cannot see FastReport `.frx` data-source bindings, procedure-to-procedure `EXEC` calls, or names composed inside a stored procedure's own body, so `unreferenced`/`missing` verdicts are scoped to what C#/Razor calls, not to what a live tenant might additionally require. **2026-08-13, M0-01-02 half A (this update):** capture tooling (`db/tools/Export-StoredProcedures.ps1`, `db/tools/list-deployed-procedures.sql`), a verification harness (`db/tools/verify-capture.sh`, tested against synthetic good/bad sample files — see [KB-060](risks/technical-debt-register.md) R-04), a DBA runbook (`db/RUNBOOK-capture-stored-procedures.md`) and a status skeleton (`db/stored-procedures/CAPTURE-STATUS.md`) now exist. **None of this changes the gap** — zero `.sql` files exist in `db/stored-procedures/` as of this update; the tooling has never been run against a database, by design (the authoring session had no credentials). **Current blocker: a named DBA with `VIEW DEFINITION` (or `db_owner`) on a nominated tenant database, and the tenant nomination itself, are still outstanding — both are human-scheduled prerequisites, not something a further AI session can supply.** Do not close this investigation until the live-database capture actually lands and is verified (a later M0-01-02 session, half C) — the tooling existing is explicitly not sufficient grounds to move this row to Completed. **2026-08-13, M0-01-02 rehearsal (same day, human-in-the-loop session):** the repo owner rehearsed the (now bug-fixed — see `Export-StoredProcedures.ps1` git history, three parameter-shape/certificate fixes for SqlServer module 22.x) capture tool against the only local database they had running, `NexGenErpDb` (`DESKTOP-FIIBE97\SQLEXPRESS`). Connection succeeded; `db/tools/list-deployed-procedures.sql` returned **zero rows** — that database has no `Sp_*` procedures at all, not even the 13 already in `Existing Store Procedures/StoredProcedures/`. No `.bak`/`.dacpac`/seed script exists anywhere in the repo to deploy procedures onto a fresh database. The repo owner does not currently know of another populated database (local, remote, demo, or backup). **This raises the blocker beyond "need a DBA": no populated tenant database is currently known to be accessible at all, by anyone.** Full detail and escalation status recorded in `db/stored-procedures/CAPTURE-STATUS.md`, "Findings requiring a human decision". Q-02 (how a tenant database is normally brought to a working state) remains unanswered and is now directly load-bearing for this investigation's ability to proceed. | [KB-102](architecture/stored-procedure-inventory.md) |
