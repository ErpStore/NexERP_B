...
> (**14 of 79**). Guards that can never refuse (**4**). Delete paths with no guard of any
> shape (**29 service files**). Guards that run **outside** the delete transaction with a
> user round-trip in the gap (**77 of 79**). None of these is what the task went looking
> for; all of them matter more to the API than the defect it did go looking for.
...
| 10 Labour Work | `LabourServices/LabourSCNService.cs` | 1638–1655 | `CanCancelAndRevokelabourSCNAsync(int scnId, int scnSubId)` | hasTransaction:1642 | — | Correct | yes | Confirmed |
...
| 11/12/13 OutSourcing, Purchase, SubContract | `OutSourcingService/Purchase_Invoice_Service/PurchaseInvoiceService.cs` | 1283–1305 | `CanDeletePurchaseInvAsync(int invId)` | — | — | Correct | yes | Confirmed |
...
| 14 Planning | `PlanningService/RcReleaseService.cs` | 803–828 | `CanDeleteRcReleaseAsync(int rcReleaseId)` | hasRouteCard:809 | hasRouteCard:815 | Correct | yes | Confirmed |
...
**Severity: High. Confidence: Confirmed by reading; Inferred as to runtime effect (§7).**
...
real defect it was chartered to find.** That is the strongest argument in this document for
scoping guard work by return shape.

**Proposed fix.** Change `hasRc` at `:614` to `hasCR`. One identifier, exactly M0-09's shape.
**Estimate 0.5 d** including a proving test in the shape of
`tests/V.SMART.Shared.Tests/Services/MfgPoServiceDeleteGuardTests.cs`. **Do not bundle it
into another change** — M0-09's value came from a two-character diff that a reviewer could
verify against a test. Proposed as task **M0-10a** (§8).
...
### 3.3 Four guards can never refuse — `Stub guard`
...
under any input.* Detected mechanically as *"no `(false, …)` return outside the `catch`
block"*.
...
> check at `:62-63` is unreachable for the same reason.

**Consequence, in domain terms.** Three delete buttons in the live Blazor UI — Grouping,
Estimation, Appointment Letter — perform an eligibility check that **always says yes**. The
user sees a guard; there is no guard. Whether that is correct depends on whether those
documents have downstream references at all, which **cannot be determined from the code** —
see **Q-64**.
...
| No header-row null check at all (queries downstream tables directly) | **20** |
...
determined — the four stub guards (§3.3), the upstream-only asymmetry (§5.1) — the code is not
...
## 7. Confidence and the one thing this audit did not do

**No test was executed, and no defect was verified empirically.** The task's acceptance
criterion *"at least one defect verified empirically, with the observed output quoted"* is
therefore **NOT MET**, and is recorded as such rather than papered over.

- The surviving R-08 instance (§3.1) is **Confirmed by reading** — the code is quoted above
  and was re-read on 2026-08-21 — and **Inferred as to runtime effect**.
- The inference is strong: M0-09 proved the **identical shape two lines away** in the same
  file, with tests observed to fail against the unfixed service, returning
  `(True, "Sales Order can be safely deleted.")`. But a proof of an adjacent instance is not
  an observation of this one.
- What would settle it: a test in the shape of
  `tests/V.SMART.Shared.Tests/Services/MfgPoServiceDeleteGuardTests.cs` seeding a
  `ContractReview` against a `MfgPoSub` with **no** Route Card, calling
  `CanSalesOrderItemCancelCheckAsync`, and observing `(True, …)`. That belongs to **M0-10a**,
  which fixes the defect; writing it here and deleting it again would produce the same
  evidence at the same cost with none of the durability.
...
|---|---|---|---|
| **M0-10a** | Fix the surviving R-08 instance — `MfgPoService.cs:614`, `hasRc` → `hasCR` — with a proving test in M0-09's shape, observed red before the fix. One identifier; nothing else in the diff. | 0.5 d | **P1** |
...
| **M0-10c** | Decide the null-handling convention (Q-61) and apply it across all 79, including the four wrong-document message strings in §3.3. Behaviour-affecting: needs the decision first. | 1 d | P2 |
...
| Date | 2026-08-21 |
| Source changes | **None.** `git diff --stat` over `V.SMART/` is empty; this task writes knowledge-base prose only. Every defect found is recorded here, not repaired. |
| Build / test run | **None, and none required.** This task compiles nothing and changes no compiled file, so a build would only re-measure an unchanged baseline. `dotnet test` is explicitly not run: no test project exists on this branch, so it would report success having executed nothing. Stated explicitly rather than left ambiguous. |
| Verification actually performed | Static analysis only: the six greps in §1.1; two independent `awk` scanners over all 93 tuple-family method bodies (declaration-liveness, §1.3; catch-behaviour and null-polarity, §4/§5.3); a per-name call-site census over all 71 guard names with injected-interface attribution; and manual re-reading of every method cited by line number in §3 and §5. |
...
| Acceptance criterion not met | *"At least one defect verified empirically."* See §7. |
