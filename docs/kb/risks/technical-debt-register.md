...
| R-69 | **The initial bundle now trips Angular's 600 kB raw warning budget.** Measured by `M2-C04-03` (2026-08-23): the single `<p-toast>` and `<app-confirm-dialog>` hosts in `app.component.html` are eager by necessity and carry `p-dialog`, `p-confirmdialog`, `app-form-field` and `app-textarea` with them, moving the initial total from 446.36 kB raw / 106.63 kB gzip to **710.39 kB raw / 158.02 kB gzip**. `npm run build` still exits 0 — it is a warning, and gzip is comfortably inside KB-050's `< 250 KB` target — but the margin to the 800 kB **error** budget is now 90 kB, and the margin is thin (**correction 2026-08-24, M2-C13**: `M2-C03` does **not** land next — it is transitively blocked behind `M0-04` via `M2-C02`→`M2-A04`). Two things were measured, not guessed: importing those two hosts from their files rather than from the `shared/components` barrel is what keeps it at 710 kB rather than **1.31 MB** (the barrel drags every form control and `decimal.js` into the initial chunk, and that build *fails*); and the remaining eager cost is the confirm-dialog host. Fix is to defer the confirm-dialog host, which needs `ConfirmDialogService` to hold the request until the host mounts, because PrimeNG's `requireConfirmation$` is a plain `Subject` and an emission before mount is simply lost | `frontend/nexgen-web/src/app/app.component.ts`, `angular.json` budgets |
