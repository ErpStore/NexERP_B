...
| R-76 | **`npm run test:ci` is intermittently red on `master` — one leak, several victims — and adding *any* spec file makes it fire more often.** `feedback/busy-overlay.component.spec.ts` renders a PrimeNG `BlockUI`, whose mask attaches to `document.body`, outside the component's own DOM, so testing-library's automatic cleanup never removes it. `@angular/build:unit-test` does not give each spec file its own document, so the orphan is still in `body` when a later file **in the same worker** runs, and everything inside it answers global role queries: its `role="progressbar"` spinner breaks `feedback/progress-bar.component.spec.ts`'s `screen.getByRole('progressbar')`, and its `role="status"` message breaks `feedback/permission-denied-state.component.spec.ts`'s `screen.getByRole('status')` — both *"Found multiple elements"*. Which files share a worker depends on how vitest partitions the file list, so **the failure rate is a function of how many spec files exist, not of what they contain**: it was intermittent at 47 files and fires on roughly half of full runs at 50. Running the three files together in isolation passes, which is why it has been invisible. **Measured 2026-08-25 by `M2-C05-01`**, on `master` at `e9a8e7a` with this branch's tree stashed: five consecutive full runs, **two clean, three red**. A second, unrelated intermittent — `form/combobox.component.spec.ts` reading `aria-expanded="true"` where it expects `false` — is timing, not leakage. **The fix is small and belongs to the shared test harness, not to any component task:** an `afterEach` that removes PrimeNG overlay hosts left in `document.body` (`document.body.querySelectorAll('.p-blockui, .p-overlay-mask').forEach((n) => n.remove())`), promoted to `angular.json`'s `setupFiles` — the same promotion `R-70` already says is overdue — or per-file isolation in the vitest configuration. `M2-C05-01` deliberately did **not** fix it: its scope is one component directory, and CLAUDE.md's *one task, one branch* makes a second task's spec file off-limits. Its own `a11y.spec.ts` scans the rendered root instead of `document.body` so it cannot become another victim. **Until this is fixed, a red `test:ci` cannot be read as a regression signal, and the frontend CI job — which is blocking — will fail on runs where nothing is wrong** | `frontend/nexgen-web/src/app/shared/components/feedback/{busy-overlay,progress-bar,permission-denied-state}.component.spec.ts`, `frontend/nexgen-web/src/app/shared/components/form/combobox.component.spec.ts`, `frontend/nexgen-web/angular.json`, `.github/workflows/ci.yml:311-313` |
