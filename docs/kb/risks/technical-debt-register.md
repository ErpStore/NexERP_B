...
| R-80     | **A DELETE endpoint whose success body is empty loses its error body's `title` — R-24 is not closed for this shape.** `ng-openapi-gen` generates `deleteCurrency()` (and every DELETE with a `204` response) with `responseType: 'text'` (`core/api/generated/fn/currency/delete-currency.ts:23`), because there is nothing to JSON-parse on success. Angular's `HttpClient` applies that same `responseType` to an **error** response too — a 409's `application/problem+json` body therefore arrives as a raw string, not an object, so `error.interceptor.ts`'s `normaliseProblem()` (`error.interceptor.ts:39-55`), which only recognises an object-shaped body, falls through to its network-failure fallback (`{ status }`, no `title`). **Observed directly, not inferred:** `M2-D01`'s own e2e delete-refusal test found the toast reading the client's generic "The currency could not be deleted." instead of the server's verbatim business-rule message it mocked. Not specific to Currency — every DELETE endpoint generated the same way (empty success body) carries the same gap. **Not fixed by `M2-D01`**: `core/api/generated/**` is outside any feature task's scope to hand-edit, and `error.interceptor.ts` is a cross-cutting M2-C02 file; the task's own instruction is to report a needed special case, not add one locally. **Two candidate fixes for whoever owns this next:** (a) have the generator emit `responseType: 'json'` for DELETE operations too, discarding the parsed body only on the 2xx success path in the generated wrapper; or (b) make `normaliseProblem()` attempt `JSON.parse` on a string body before falling back. Recorded in `M2-D01`'s Close-out and `task-tracker.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `frontend/nexgen-web/src/app/core/api/generated/fn/currency/delete-currency.ts`, `frontend/nexgen-web/src/app/core/http/error.interceptor.ts`, `frontend/nexgen-web/e2e/currency.spec.ts`                                                                |
...
| R-81     | **`DocumentEditorConfig` declares a `subLines` descriptor that the shell records but does not render.** The M2-C08-01 layout survey (INV-065) found **no** line → sub-line grid in any of the four sampled document screens, and the single nesting case in `ItemUpsert.razor` is master → detail → sub-grid (`:1070` → `EditRM` `:1100` → second `EditForm` `:1122` → process grid `:1330`), not line → sub-line. The task nonetheless required the contract to accommodate the survey answer *now*, so the descriptor is typed and exported; **rendering it needs row expansion inside `LineItemGrid`, which M2-C08-01’s own *Files that must not change* list forbids it from touching**. Consequence: a future module that genuinely needs sub-lines must raise a **change request against M2-C07** (row expansion, or a per-row detail slot) before the descriptor does anything. The binding shape is already fixed, so that change is additive rather than a contract break. No module is blocked today — five of ~65 screens were sampled, and none of the four documents needs it | `frontend/nexgen-web/src/app/shared/components/document-editor/document-editor.model.ts` (`DocumentSubLineGrid`), `frontend/nexgen-web/src/app/shared/components/line-item-grid/line-item-grid.component.ts` |
| R-82     | **`npm run format:check` fails on `master` on two files that belong to no current task.** Measured 2026-08-28 on `master` with a clean tree: `prettier --check .` warns on `frontend/nexgen-web/eslint.config.js` and `frontend/nexgen-web/src/app/shared/utils/decimal/no-float-money.spec.ts` (whitespace only — an over-long `no-restricted-imports` object literal, and one wrapped line). Every acceptance-criteria list that names `npm run format:check` therefore fails for a reason unrelated to the task being validated, which is exactly how a real formatting regression would go unnoticed. **The fix is one command, `npm run format`**, but it rewrites two files owned by M2-C10 / M2-C04-01, so M2-C08-01 recorded it rather than folding an unrelated diff into a single-scope commit. **✅ RESOLVED 2026-08-28.** The owner answered `Q-105` with option (a): branch `hygiene/prettier-format-check` ran `npm run format` on exactly those two files and nothing else, and was merged to `master` at `5a543aa` (`--no-ff`, no conflicts). Verified on merged `master` and again on this branch after taking `master` in: `prettier --check .` reports "All matched files use Prettier code style". `master`'s blocking CI Format-check job is green for the first time since `1c93bb3`, and no branch inherits the failure any more | `frontend/nexgen-web/eslint.config.js`, `frontend/nexgen-web/src/app/shared/utils/decimal/no-float-money.spec.ts` |
| R-83     | **`npm run test:ci` is intermittently red under load, in files that pass in isolation, and it is red on `master` too.** Measured 2026-08-28 (M2-C08-01): four full runs on the task branch gave **two green** (`713/713`) and two red (15 and 22 failed) — and two baseline runs with the branch stashed gave **one green** (`685/685`) and one red (2 failed), so the condition pre-dates this task. Every failure observed is a `Test timed out in 5000ms` in a **pre-existing** spec (`form/date-picker`, `form/text-input`, `form/multi-select`, `form/radio-group`, `overlay/modal`, `overlay/drawer`, `overlay/confirm-dialog.deferred`, `header`, `features/auth/login`, `line-item-grid.render-performance`); **no `document-editor` spec failed in any run**. The pattern is elapsed-time, not logic: the same files take 15–36 s in a red run against ~2 s in a green one, i.e. the 5 s per-test default is being eaten by machine load in a suite that shares one worker per file (R-76 records the related sharing hazard). **Consequence:** any acceptance criterion of the form *"`npm run test:ci` passes"* is a coin-flip on a loaded machine, and a real regression will be dismissed as "the flaky suite". **Candidate fixes, none taken here** (this is a workspace-wide test-configuration decision, not a component task's): raise `testTimeout` in the builder options, or set `isolate: true`, or bound worker concurrency — each has a runtime cost that whoever owns CI should weigh | `frontend/nexgen-web/angular.json` (`test` target), `frontend/nexgen-web/src/test-setup.ts` |
| R-84     | **`ContactPersonMapping` maps the wrong type pair — a copy-paste that left `ContactPersonVM → ContactPerson` undefined and `CustomerIndirectVM → CustomerIndirect` declared twice.** `V.SMART/V.SMART.Shared/Mappings/MasterMapping/GeneralMasterProfile/ContactPersonMapping.cs:21` (pre-M2-D02-01) declared `CreateMap<CustomerIndirectVM, CustomerIndirect>()` under a `// VM -> Entity` comment, duplicating the identical map already made by `CustomerIndirectMapping.cs:21` — so the profile whose whole purpose is contact persons defined no VM→entity map for them at all. Found 2026-08-28 by `M2-D02-01`, which needed that map for the authorised BR-CUST-017 fix. **What was done:** the missing `CreateMap<ContactPersonVM, ContactPerson>()` was added (in scope — the fix depends on it); **the erroneous duplicate was left exactly as found** (out of scope, and removing it is a behaviour question, not a tidy-up: AutoMapper's duplicate-map handling for two identical registrations across two profiles is not something this task observed). **Why it matters:** the application boots today, so the duplicate is evidently tolerated by AutoMapper 16.1.1's configuration build — but the same copy-paste shape is worth grepping for across the other ~190 mapping profiles before a future upgrade turns tolerance into a `DuplicateTypeMapConfigurationException` at startup, which would be a total outage rather than a mapping bug. No investigation of the other profiles was performed | `V.SMART/V.SMART.Shared/Mappings/MasterMapping/GeneralMasterProfile/ContactPersonMapping.cs`, `.../CustomerIndirectMapping.cs` |
