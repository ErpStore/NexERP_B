...
| 3 | A database built by **replaying migrations** from `InitialCreate` still receives the row, because migration history is never rewritten. | Same runbook, section 1 |
| 4 | **No bootstrap component exists** to create the first administrator on an empty database. Option A (runtime bootstrap from an explicitly supplied secret, failing startup loudly if absent) was chosen in principle but **deferred** — it needs coordination with M0-03-03's fail-fast startup validation and is larger than M0-06's budget. Until it exists, a from-model database has no way in. | Follow-up task, not yet numbered |
...
land M0-05; then raise the Option-A bootstrap task. **Do not close R-09** until all three are
done.
