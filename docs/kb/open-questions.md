...
|---|---|---|---|
| **Q-19** | **Is the public visibility of `https://github.com/ErpStore/NexERP_B.git` intended?** `git ls-remote` succeeds without authentication (Confirmed, INV-029, 2026-08-12), and commit `c12c5b2` contains the SA password, the production host `154.61.76.112,1533` and the `bspl` production credential in four files. **Escalated in writing 2026-08-12** (M0-00) to **Kumar**, confirmed owner of `ErpStore/NexERP_B`, with R-01/R-02 evidence attached ([KB-085](execution/M0-00-baseline-decisions.md#q-19-escalation-public-repo--published-credentials)). Current visibility recorded as **public**, re-confirmed the same day. **The decision on whether to change visibility is still outstanding** — M0-00 does not act on it. | Determines whether the exposed credentials must be treated as already harvested by third parties, and whether an incident response — not just a rotation — is required. | **M0-04, immediately.** Also M0-05 |
