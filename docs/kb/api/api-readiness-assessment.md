...
author one _and_ an AutoMapper profile before `<W>-06`.
...
| A3  | ~~**Tenant resolution for a cross-origin SPA**~~ — **DELIVERED by M2-A05 (2026-08-27, `Needs Review`).** Tenant in the login request (`{ tenant, username, password }`, ADR-002 §5's own recommended option A, exactly); JWT claim thereafter, unchanged; real per-environment CORS origin list (`CorsOptions`, empty in production by default — fails closed, not open). See [KB-040](api-overview.md#post-apiv1authlogin), [multi-tenancy](../architecture/multi-tenancy.md) problems 1–3, all closed. **Not delivered, tracked separately:** real production CORS origin values — Q-16 (deployment topology) is explicitly deferred, so the task ships the configuration mechanism only, per its own Prerequisites                                                                                                                                      | Host-based resolution breaks for an SPA                                                                   | 3–5 days |
| A4  | ~~**Refresh tokens + revocation**~~ — **DELIVERED by M2-A04 (2026-08-27, `Needs Review`).** `POST /api/v1/auth/refresh` (one-time-use rotation) and `POST /api/v1/auth/logout` (revoke); access-token lifetime cut from 480 to 15 minutes; `IsActive` re-checked on every rotation. Tokens stored SHA-256-hashed in a new per-tenant `RefreshTokens` table (one EF migration, additive only). See [KB-040](api-overview.md#post-apiv1authrefresh--added-by-m2-a04-2026-08-27), [KB-013](../architecture/auth-and-permissions.md#1-authentication). **Not delivered here, tracked separately:** refresh-token transport stays request-body, not an `HttpOnly` cookie, pending Q-16 (deployment topology); the 200-path has not been exercised against a live host (R-43, controller-level tests only)                                                       | 8-hour non-revocable JWT is unacceptable for an ERP                                                       | 3–5 days |
...
- **The Excel endpoints are a reference implementation, not a rollout.** They exist on
  `currencies` only. Item **B4**'s Excel half is therefore _proven_, not _done_; rolling the
...
sweep. Read B9 as _contract done, rollout ~1/134_.
...
2. **Commands are explicit.** Anything that is a workflow step (cancel, short-close,
   approve, release, post) gets its own `POST /{id}/{verb}` endpoint that runs the _entire_
...
[`api-overview.md` § _Error contract_](api-overview.md#error-contract-m2-a06).
