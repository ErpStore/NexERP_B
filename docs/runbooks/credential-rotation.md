...
| **C-2** | SQL login on production host `154.61.76.112,1533` (databases `IQSmartDb_Master`, `IQSMARTDEMO_DB_2025-26`) | Same consumers as C-1 — it was the commented alternative in the same files. **No longer in any `V.SMART/` file**; still in git history, and its password is still in `HEAD` at `docs/kb/risks/technical-debt-register.md:44`. | Confirmed |
...
| SQL Agent jobs, backup/restore scripts, monitoring agents | **Unknown** — this is **Q-15** in `docs/kb/open-questions.md:72`, still open. INV-022 confirmed the *application* has no background processing; that says nothing about jobs configured on the SQL Server instance itself. Resolved by the DBA enumerating logins, jobs and their sessions on `154.61.76.112,1533`. |
...
## 3. C-1 and C-2 — the SQL logins

**Owner.** Whoever holds `sysadmin` (or equivalent) on the production host
`154.61.76.112,1533` (C-2) and on the local/dev SQL Express instance (C-1). Name that person
before starting; the checklist requires their name.
...
**Window.** A maintenance window is **required** for C-2. C-1 does not affect tenants *unless
a `Tenants` row points at that instance* — check first; do not assume it does not.
...
|---|---|---|---|---|
| 1 | The old SQL login (C-1 and/or C-2, as applicable) can **no longer authenticate** to its instance — demonstrated by a failed connection attempt, not by inspection. | | | |
