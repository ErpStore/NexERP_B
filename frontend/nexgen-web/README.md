...
- Node **22 LTS** (`.nvmrc`). See *Node version* below — `engines` is `>=22`, deliberately
...
*and* in `environment.prod.ts:3` — its production build points at localhost. Do not reproduce
...
are never mixed. The live Blazor UI loads MudBlazor 8 *and* Bootstrap 5 simultaneously; that
